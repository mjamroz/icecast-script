...
port = 8000
password = "password"
jingle_after = 4 #play jingle after 4 songs
...
# you need libshout binding to python. You can download source from icecast.org
...
class RunStream (threading.Thread):
   def __init__ (self, channel_mount, music_directory, station_url, genre,name, description, bitrate="128", samplerate="44100", channels="2",music_format="mp3",jingle_dir=""):
      ...
      while 1: #infinity
         ...
         for e in self.files_array:
            ...
            if(self.song_counter%jingle_after==0):
               self.sendfile(choice(self.jingle_files)) # play random jingle
